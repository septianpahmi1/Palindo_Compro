                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">DataTable Salary</h3>
                        ...
                        @include('dashboard.salary.create')
                        <!-- /.card-header -->
                        <div class="card-body">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group btn-block">
                                                    @if ($item->status == 'Pending')
                                                        <button url="{{ route('salary.status', $item->id) }}"
                                                            data-id="{{ $item->id }}" type="button"
                                                            class="btn btn-primary btn-sm status"><i
                                                                class="fas fa-check"></i></button>
                                                    @endif
                                                    <a href="{{ route('salary.invoice', $item->id) }}" target="_blank"
                                                        type="button" class="btn btn-success btn-sm"><i
                                                            class="fas fa-file-invoice"></i></a>
                                                    <button url="{{ route('salary.delete', $item->id) }}"
                                                        data-id="{{ $item->id }}" type="button"
